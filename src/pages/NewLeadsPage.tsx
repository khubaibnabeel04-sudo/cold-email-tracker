...
import { Upload, Plus, Trash2, Edit3, X, Save, Search, RefreshCw, Download } from 'lucide-react';
...
export default function NewLeadsPage() {
  ...
  async function processFile(file: File) {
    ...
    try {
      ...
      dispatch({ type: 'ADD_LEADS', payload: { leads, page: 'new' } });
      alert(`Import complete! Added ${leads.length} leads. Skipped ${skippedCount} duplicates/empty records.`);
    } catch (err: any) {
  ...
  function handleAddLeadSubmit(e: React.FormEvent) {
    ...
    dispatch({
      type: 'ADD_LEADS',
      payload: { leads: [newLead], page: 'new' }
    });
  ...
  function removeLead(id: string) {
    ...
    dispatch({ type: 'REMOVE_LEAD', payload: { id, page: 'new' } });
  }
  ...
  async function syncSingleLead(lead: Lead) {
    ...
    setSingleSyncLogs([`[${new Date().toLocaleTimeString()}] Starting status sync for ${lead.email}...`]);
    try {
      ...
      // Skip stale/reset rules for replied or draft leads — those are active conversations
      ...
      if (fu2Check.shouldMoveToStale) {
        dispatch({ type: 'MOVE_TO_STALE', payload: { id: lead.id, sourcePage: 'new', updates } });
        movedToStale = true;
        ...
        if (lead.syncedOnce && daysSince > 60) {
          // Re-sync >60 days → move lead to Stale Leads page
          dispatch({ type: 'MOVE_TO_STALE', payload: { id: lead.id, sourcePage: 'new', updates } });
          movedToStale = true;
          setSingleSyncLogs(prev => [...prev, `[${new Date().toLocaleTimeString()}] -> Lead last contacted ${Math.floor(daysSince)} day(s) ago (> 60 day cutoff). Moving to Stale Leads.`]);
        } else if (!lead.syncedOnce && daysSince > 30) {
          // First sync >30 days → reset to new in place
          setSingleSyncLogs(prev => [...prev, `[${new Date().toLocaleTimeString()}] -> Lead last contacted ${Math.floor(daysSince)} day(s) ago (> 30 day cutoff). Resetting status to: new.`]);
          updates.status = 'new';
          ...
          setSingleSyncLogs(prev => [...prev, `[${new Date().toLocaleTimeString()}] -> Lead last contacted ${Math.floor(daysSince)} day(s) ago (< ${lead.syncedOnce ? 60 : 30} day cutoff). Keeping status: ${updates.status}.`]);
        }
      }
  ...
  function exportToCSV() {
    ...
    link.href = url;
    link.setAttribute('download', `new-leads-export-${new Date().toISOString().split('T')[0]}.csv`);
  ...
  const filteredLeads = state.newLeads.filter(lead => {
    const matchesSearch = 
  ...
  return (
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24 }}>
        <div>
          <h1 style={{ fontSize: 28, fontWeight: 700 }}>New Leads</h1>
          <p style={{ color: 'var(--text-secondary)' }}>{state.newLeads.length} leads</p>
        </div>
      ...
      <div style={{ 
        ...
        <div style={{ position: 'relative', flex: 1, minWidth: 240 }}>
          <input
            type="text"
            placeholder="Search leads by name, email, or details..."
            value={searchQuery}
        ...
        <div style={{ display: 'flex', gap: 6, overflowX: 'auto', paddingBottom: 2 }}>
          {(() => {
            ...
            const labels: Record<string, string> = {
              all: 'All Leads', new: 'New', draft: 'Draft', initial_sent: 'Initial Sent', replied: 'Replied'
            };
          ...
          <thead>
            <tr style={{ borderBottom: '2px solid var(--border)', textAlign: 'left' }}>
              ...
              {state.columns.filter(c => !['name', 'email'].includes(c)).map(col => (
                <th key={col} style={{ padding: 12, fontWeight: 600 }}>{col}</th>
              ))}
                ...
                >
                  ...
                  <td style={{ padding: 12 }}>
                    <span style={{
                      ...
                      fontSize: 12,
                      fontWeight: 600,
                      background: lead.status === 'new' ? 'var(--bg-hover)' :
                                  lead.status === 'replied' ? 'var(--green-bg)' : 'var(--blue-bg)',
                      color: lead.status === 'new' ? 'var(--text-primary)' :
      ...
      {/* POPUP MODAL: Add Individual Lead */}
      {showAddLeadModal && (
        <div style={{
          position: 'fixed',
          top: 0,
